using Microsoft.AspNetCore.Mvc;
/// <summary>
/// 演習-04 [FormQuery]属性を利用してクエリパラメータを受け取る
/// </summary>
/// ルーティング設定(ベースURL)
[Route("Exercise04")]
public class Ex04Controller : Controller
{
    /// <summary>
    /// リクエストパラメータを加算した結果を返す
    /// </summary>
    /// <param name="value1">加算対象</param>
    /// <param name="value2">加算対象</param>
    /// <returns>加算結果</returns>

    [HttpGet("Calc")]/// 属性(ルーティング設定)
    // IActionResultは戻り値の型(インターフェース)：複数の返し方があるときに有効
    // [FromQuery]:QueryはURLが入力元になる(バインディング属性)
    public IActionResult Calc([FromQuery] int value1, [FromQuery] int value2)
    {
        // 演習-05 型変換エラーの場合、エラーメッセージを出力する
        // ModelStateプロパティを利用した型変換エラーチェック
        /** 追加 **/
        if (!ModelState.IsValid) // 状態管理オブジェクト(型変換エラーを見る)
        {
            // value1とvalue2で型変換エラー
            if ((ModelState["value1"]?.Errors.Count ?? 0) > 0
            && (ModelState["value2"]?.Errors.Count ?? 0) > 0)
            {
                // ActionResult: 文字列をそのままレスポンスとして返す
                return Content("value1とvalue2は整数ではありません。");
            }
            // value1で型変換エラー
            if ((ModelState["value1"]?.Errors.Count ?? 0) > 0)
            {
                return Content("value1は整数ではありません。");
            }
            // value2で型変換エラー
            if ((ModelState["value2"]?.Errors.Count ?? 0) > 0)
            {
                return Content("value2は整数ではありません。");
            }
        }
        var result = value1 + value2;
        return Content($"{value1} + {value2} = {result}");
    }
    /// <summary>
    /// オリジナル【文字型バージョン】
    /// </summary>
    /// <param name="word3"></param>
    /// <param name="word4"></param>
    /// <returns></returns>
    [HttpGet("Calc2")]
    public IActionResult Calc2([FromQuery] string word3, [FromQuery] string word4)
    {
        if (!ModelState.IsValid)
        {
            if ((ModelState["value3"]?.Errors.Count ?? 0) > 0
            && (ModelState["value4"]?.Errors.Count ?? 0) > 0)
            {
                return Content("value3とvalue4は文字ではありません。");
            }
            if ((ModelState["value3"]?.Errors.Count ?? 0) > 0)
            {
                return Content("value3は文字ではありません。");
            }
            if ((ModelState["value4"]?.Errors.Count ?? 0) > 0)
            {
                return Content("value4は文字ではありません。");
            }
        }
        var result = word3 + word4;
        return Content($"{word3} +  {word4} = {result}");
    }
}